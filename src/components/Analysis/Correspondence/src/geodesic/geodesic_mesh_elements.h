...
public:
	...
	SurfacePoint(face_pointer f, float x, float y, float z): m_p(f)
	{
	  set(x,y,z);  
	}
	 ...
	 * 	Vincent Léon
	 * 	vincent.leon@lifl.fr
	 *	r1 and r2 are typically random
	 */
	SurfacePoint(face_pointer f, float r1, float r2) : m_p(f)
	{
	  set(0,0,0);
	  Point3D vA;
	  Point3D vB;
	  ...
	  vA.add(f->adjacent_vertices()[0]);
	  vB.add(f->adjacent_vertices()[1]);
	  vC.add(f->adjacent_vertices()[2]);
	  vA.multiply(1-sqrt(r1));
	  vB.multiply(sqrt(r1)*(1-r2));
	  vC.multiply(sqrt(r1)*r2);
	  this->add(&vA);
	  this->add(&vB);
	  this->add(&vC);
	};
